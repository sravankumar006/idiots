...
export default function DashboardClient({ activeUser, targetUserId }: DashboardClientProps) {
  ...
  const fetchInvitations = async () => {
    ...
    const { data, error } = await supabase
      ...
      .eq('invitee_user_id', activeUser.id)
      .eq('status', 'pending')
  ...
  useEffect(() => {
    if (activeUser && !isReadOnly) {
      ...
      const sub = supabase.channel('dashboard-study-invitations')
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'study_room_invitations', filter: `invitee_user_id=eq.${activeUser.id}` },
          () => {
            fetchInvitations()
          }
        )
  ...
  }, [supabase, activeUser, isReadOnly])
        ...
        <div className="mb-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {pendingInvitations.map((invite) => {
              ...
              const hostAvatar = AVATAR_MAP[hostProfile?.avatar || 'avatar-cyber-ghost'] || { gradient: 'from-[#3A3530] to-[#2B2824]', symbol: 'EX' }
